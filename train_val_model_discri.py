...
def rl_loss(output, target, discriminator):
    reward = discriminator(output).detach()
    pg_loss = -reward * F.log_softmax(output, dim=-1).gather(1, target.unsqueeze(-1))
    return pg_loss.mean()
# nltk.download('punkt')
...
def fit(model, train_loader, optimizer, criterion):
    ...
    for i, (source, target, attention_mask) in prog_bar:
        ...
        optimizer.zero_grad()
        loss = rl_loss(out, target_out, discriminator)
